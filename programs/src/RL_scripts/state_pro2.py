...
#Also Provides Dist To Goal : edited on 26 May 2022
...
def callback(data):
    ...
    goal_dist_pub = rospy.Publisher("Dist_to_Goal/robot2" , Float64 , queue_size =10)
    cluster_pub = rospy.Publisher("Clustered_Obstacles/robot2" , PointCloud2 , queue_size  = 10)
    ...
    #print(timer)
    global listener
    listener.waitForTransform("/camera_link" , "/Atool0", timer , rospy.Duration(4.0))
	...
	laser_point.header.frame_id = "/Aforearm_link"
	...
	p = listener.transformPoint("Abase_link" ,min_dist_point)
	...
	laser_point.header.frame_id = "/Aupper_arm_link"
	...
	laser_point.header.frame_id = "/Awrist_1_link"	
	...
	laser_point.header.frame_id = "/Atool0"
    ...
    nearest_obs  = np.zeros(NUMBER_POINTS , dtype=[('x' , np.float32), ('y', np.float32), ('z' , np.float32),('distances' , np.float32)])
    ...
    print(nearest_obs)
    least_distance_msg.header.frame_id = "Abase_link"
    least_distance_pub = rospy.Publisher("/Least_distance/robot2" , PointCloud2 , queue_size = 10)
    ...
    nearest_obs_msg.header.frame_id = 'Abase_link'
    pub_nearest_obs = rospy.Publisher("/RL_States/Nearest_Obstacles_States/robot2" , PointCloud2 , queue_size=10)
    ...
    goal_dist_pub.publish(np.linalg.norm(a))
...
def start():
    rospy.init_node("clusterer_robot2")
